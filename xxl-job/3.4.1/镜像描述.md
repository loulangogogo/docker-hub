...
该镜像使用外部 mysql 数据库，需要先创建一个库表，可以参考文档创建，或查看 [xxl-job.sql(注意选择版本)](https://github.com/xuxueli/xxl-job/blob/3.4.1-release/doc/db/tables_xxl_job.sql)。

### docker

> docker run --name xxl-job -e DB_HOST=127.0.0.1 -e DB_PORT=3306 -e DB_USER=root -e DB_NAME: xxl_job -e DB_PASSWORD=root -p 8080:8080 osmiling/xxl-job:3.4.1
